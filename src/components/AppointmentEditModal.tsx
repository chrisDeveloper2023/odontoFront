...
const AppointmentEditModal: React.FC<AppointmentEditModalProps> = ({
    ...
    async function load() {
      try {
        ...
        const [pacData, conData, citaData] = await Promise.all([
          ...
          apiGet<any>(`/citas/${appointmentId}`),
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      toast.error((err as Error).message || "Error al actualizar la cita");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {loading && (
          <div className="flex items-center justify-center py-8">
            <div className="text-muted-foreground">Cargando datos de la cita...</div>
          </div>
        )}
