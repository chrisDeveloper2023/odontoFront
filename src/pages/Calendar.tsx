...
const Calendar: React.FC = () => {
  ...
  const citasEjemplo: Cita[] = [
    ...
    // Citas para el día actual (ejemplo)
    {
      id: 9,
      paciente: "Mía Barco",
      tipo: "Resinas",
  ...
  useEffect(() => {
    const cargarDoctores = async () => {
      ...
      try {
        ...
        // Inicializar odontólogos con colores por defecto
        ...
        // Actualizar citas de ejemplo con nombres reales de médicos
  ...
  const obtenerDiasMes = () => {
    ...
    for (let dia = 1; dia <= ultimoDia.getDate(); dia++) {
      diasActuales.push(new Date(año, mes, dia));
    }
    
    // Días del mes siguiente para completar la última semana
    const diasSiguientes = [];
    const totalCeldas = diasAnteriores.length + diasActuales.length;
    const celdasRestantes = 42 - totalCeldas; // 6 semanas * 7 días = 42 celdas
    for (let dia = 1; dia <= celdasRestantes; dia++) {
      diasSiguientes.push(new Date(año, mes + 1, dia));
    }
  ...
  const obtenerCitasDelDia = (fecha: Date) => {
    ...
    return citas.filter(cita => {
      ...
      return fechaCita.getFullYear() === año && 
